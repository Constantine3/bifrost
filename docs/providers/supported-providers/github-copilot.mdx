...
Server-to-server access needs four things set up on the GitHub side. All four are
...
3. Enable the organization policy **Allow use of Copilot CLI billed to the organization**.
4. Enable the organization for **Copilot requests from GitHub App installations**.

Then collect: the App ID (or Client ID), the installation ID, the numeric ID of any
repository the installation can access, and the App's private key PEM.
