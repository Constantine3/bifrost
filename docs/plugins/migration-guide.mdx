...
Only if it touches credentials. A plugin that logs, rewrites a body, adds a tracing header, or
short-circuits on a business rule needs no change - and gains something: `HTTPTransportPreHook` now sees
the resolved caller identity on `ctx`, because authentication has already run.
...
A plugin may export both hooks. They are independent phases and each runs once per request.
